...
static int fb_rotate;
static struct fb_var_screeninfo fb_vinfo;
static struct fb_fix_screeninfo fb_finfo;
static void* fb_pixels;
static void* fb_pixels_real;
...
static int fb_softdoublebuffer;
static int fb_doublebuffer;
static int fb_rowbytes;
...
static int gfx_show_primary(void)
{
    fb_frontbuffer_id = 0;
...
static void gfx_unmap_buffer(void)
{
    if (fb_pixels_real)
    {
        if (munmap(fb_pixels_real, fb_finfo.smem_len))
        {
            perror(fb_dev);
        }
        fb_pixels_real = NULL;
...
static void* gfx_map_buffer(void)
{
    if (fb_fd == -1)
    {
        return NULL;
    ...
    if (fb_rotate > 0)
    {
        fb_pixels_real = mmap(NULL, fb_finfo.smem_len, PROT_WRITE, MAP_SHARED, fb_fd, 0);
        fb_screensize = fb_rowbytes * fb_yres;
        fb_pixels = malloc(fb_screensize);

        if (fb_pixels_real == (void*)(MAP_FAILED))
        {
            perror(fb_dev);
            fb_pixels = NULL;
            return NULL;
    ...
    fb_softdoublebuffer = 0;
    if (fb_rotate>0)
        fb_softdoublebuffer = 1;

    return fb_pixels;
}

static void gfx_dump_info(void)
{
    dprintf(STDERR_FILENO, "%s:xres=%d yres=%d\n", fb_dev, fb_xres, fb_yres);
    dprintf(STDERR_FILENO, "%s:bpp=%d\n", fb_dev, fb_vinfo.bits_per_pixel);
    dprintf(STDERR_FILENO, "%s:line=%d\n", fb_dev, fb_finfo.line_length);
}
...
int gfx_open(unsigned* xres, unsigned* yres, size_t* rowbytes)
{
    ...
    fb_rotate = getenv("FBROTATE") ? atoi(getenv("FBROTATE")) : 0;
    ...
    fb_rowbytes = fb_finfo.line_length;
    if (fb_rotate % 2 != 0)
    {
        *xres = fb_yres;
        *yres = fb_xres;
        fb_rowbytes = fb_yres * 2;
    }
...
void* gfx_buffer(size_t* len)
{
    ...
    if ( fb_softdoublebuffer || !fb_doublebuffer || !fb_frontbuffer_id)
    {
        return fb_pixels;
    }
...
static void gfx_flip_frame(int rotate)
{
    ...
    if (fb_rotate == 1)
    {
        ...
        while ( xx < xmax )
        {
            ...
            while ( yy < ymax )
            {
                ((uint16_t*)fb_pixels_real)[(xx) + ((yy) * (fb_xres))] = ((uint16_t*)fb_pixels)[(yy) + ((xmax-xx) * (fb_yres))];
                yy++;
            }
            xx++;
        }
